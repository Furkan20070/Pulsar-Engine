...
{
	if (SetWindowText(hwnd, CharToWide(title.c_str()).c_str()) == 0)
	{
		throw PLSR_LAST_EXCEPT();
	}
}
...
std::wstring Window::Exception::TranslateErrorCode(HRESULT hr) noexcept
{
	...
	const DWORD nMsgLen = FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
		nullptr, hr, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), reinterpret_cast<LPWSTR>(&pMsgBuf), 0, nullptr);
	if (nMsgLen == 0)
	{
...
std::wstring Window::CharToWide(const char* text)
{
	if (!text)
		return L"";
...
std::string Window::WideToUtf8(const std::wstring& w)
{
	if (w.empty())
		return {};
...
{
	return WideToUtf8(Exception::TranslateErrorCode(hr).c_str());
}

const char* Window::NoGfxException::GetType() const noexcept
{
	return "Pulsar Window Exception [No Graphics]";
}